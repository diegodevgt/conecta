import { CCol, CContainer, CRow } from "@coreui/react";
import './componentes.css';
import { useEffect } from 'react';
import { useState } from 'react';
import { isNil } from "lodash";

const Event = (props) => {
    const { informacionPedido } = props;

    const [data, setData] = useState([{
        clase: 'completado',
        img: '<svg id="Capa_1" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" fill="#264782" viewBox="0 0 100 100"><defs><style>.cls-1{fill:#264782;}</style></defs><path class="cls-1" d="M51.18,82a26.1,26.1,0,0,0-2.59-1.3Q37,75.13,25.47,69.59c-2-1-1.76-.69-1.76-2.79q0-14.89,0-29.78a1.41,1.41,0,0,1,.91-1.44q8-3.82,16-7.68L50.82,23a1.73,1.73,0,0,1,1.65,0q11,5.31,22,10.59c1.4.67,2.79,1.35,4.2,2A1.4,1.4,0,0,1,79.59,37q0,15.38,0,30.76a1.4,1.4,0,0,1-.93,1.42q-11.41,5.46-22.81,11c-1.25.6-2.54,1.15-3.74,1.85ZM26.81,36.62l.51.27L45.05,45.4c.89.43,1.17.91.89,1.48s-.82.64-1.72.2l-18-8.67c-.63-.31-.63-.31-.63.39q0,14,0,27.93a1.13,1.13,0,0,0,.74,1.19Q38.15,73.59,50,79.32c.67.32.67.32.67-.41v-28a1.08,1.08,0,0,1,0-.18.7.7,0,0,0-.49-.81c-.53-.22-1.05-.49-1.57-.75s-.85-.79-.61-1.3.78-.66,1.45-.36c.49.23,1,.44,1.46.72a1.35,1.35,0,0,0,1.41,0c1.75-.88,3.51-1.72,5.28-2.55A.94.94,0,0,1,59,46.1c.23.49,0,.94-.57,1.27-.14.08-.28.14-.42.21-1.64.79-3.27,1.59-4.92,2.36a.76.76,0,0,0-.5.8q0,14.17,0,28.34c0,.46.11.53.52.32,1.27-.63,2.54-1.23,3.82-1.84l20.19-9.7c.36-.17.64-.32.64-.83q0-14.19,0-28.39c0-.52-.17-.47-.54-.29-2.26,1.1-4.52,2.19-6.8,3.26a.93.93,0,0,0-.62,1c0,3.1,0,6.2,0,9.31a1.35,1.35,0,0,1-.86,1.4C67,54.27,65,55.23,63,56.17c-1,.45-1.55.11-1.63-.94,0-.21,0-.42,0-.63,0-3,0-5.94,0-8.91a.82.82,0,0,0-.55-.88c-1.39-.63-2.76-1.31-4.14-2L35.79,32.77a1,1,0,0,0-1,0c-1.2.59-2.4,1.15-3.61,1.73Zm49.67,0-.63-.32L71,34q-9.44-4.54-18.86-9.09a1,1,0,0,0-1,0c-1.91.94-3.83,1.86-5.74,2.79a1.56,1.56,0,0,0-.72.43l.56.29,8.22,3.94q7.71,3.7,15.41,7.43a1.13,1.13,0,0,0,1.14,0c.85-.43,1.71-.82,2.56-1.24Zm-39-5.1L41,33.21q10.41,5,20.81,10a1,1,0,0,0,1,0c1.26-.63,2.54-1.22,3.81-1.84.17-.09.42-.1.48-.38-.11-.06-.2-.12-.31-.17l-16.68-8-7.17-3.43a.81.81,0,0,0-.77-.12ZM63.3,49.44c0,1.33,0,2.66,0,4,0,.42.09.52.49.32,1.23-.61,2.47-1.2,3.72-1.78a.66.66,0,0,0,.42-.67c0-2.64,0-5.29,0-7.93,0-.4-.09-.52-.5-.32-1.21.6-2.43,1.19-3.66,1.76a.72.72,0,0,0-.47.76C63.31,46.86,63.3,48.15,63.3,49.44Z"/><path class="cls-1" d="M28.47,58.1a5.06,5.06,0,0,1,.53.19l7.11,3.41a1.78,1.78,0,0,1,.3.17A.85.85,0,0,1,36.79,63a.86.86,0,0,1-1.15.49,12,12,0,0,1-1.37-.62L28.22,60a1.86,1.86,0,0,1-.45-.27.85.85,0,0,1-.27-1A.93.93,0,0,1,28.47,58.1Z"/><path class="cls-1" d="M33.21,65a.88.88,0,0,1-1.13.89,6.38,6.38,0,0,1-1-.43c-1-.46-1.95-.93-2.92-1.41-.65-.32-.88-.84-.62-1.34a1,1,0,0,1,1.4-.35c1.23.57,2.44,1.16,3.66,1.75A1,1,0,0,1,33.21,65Z"/></svg>',
        estado: 'Recibido',
        detalles: ['Conecta Guate - Pruebas', `${(new Date()).toLocaleString('es-GT', { hour12: false })}`]
    }]);

    // eslint-disable-next-line react-hooks/exhaustive-deps
    const iconosImg = {
        /*recibido*/ "Recibido": '<svg id="Capa_1" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" fill="#264782" viewBox="0 0 100 100"><defs><style>.cls-1{fill:#264782;}</style></defs><path class="cls-1" d="M51.18,82a26.1,26.1,0,0,0-2.59-1.3Q37,75.13,25.47,69.59c-2-1-1.76-.69-1.76-2.79q0-14.89,0-29.78a1.41,1.41,0,0,1,.91-1.44q8-3.82,16-7.68L50.82,23a1.73,1.73,0,0,1,1.65,0q11,5.31,22,10.59c1.4.67,2.79,1.35,4.2,2A1.4,1.4,0,0,1,79.59,37q0,15.38,0,30.76a1.4,1.4,0,0,1-.93,1.42q-11.41,5.46-22.81,11c-1.25.6-2.54,1.15-3.74,1.85ZM26.81,36.62l.51.27L45.05,45.4c.89.43,1.17.91.89,1.48s-.82.64-1.72.2l-18-8.67c-.63-.31-.63-.31-.63.39q0,14,0,27.93a1.13,1.13,0,0,0,.74,1.19Q38.15,73.59,50,79.32c.67.32.67.32.67-.41v-28a1.08,1.08,0,0,1,0-.18.7.7,0,0,0-.49-.81c-.53-.22-1.05-.49-1.57-.75s-.85-.79-.61-1.3.78-.66,1.45-.36c.49.23,1,.44,1.46.72a1.35,1.35,0,0,0,1.41,0c1.75-.88,3.51-1.72,5.28-2.55A.94.94,0,0,1,59,46.1c.23.49,0,.94-.57,1.27-.14.08-.28.14-.42.21-1.64.79-3.27,1.59-4.92,2.36a.76.76,0,0,0-.5.8q0,14.17,0,28.34c0,.46.11.53.52.32,1.27-.63,2.54-1.23,3.82-1.84l20.19-9.7c.36-.17.64-.32.64-.83q0-14.19,0-28.39c0-.52-.17-.47-.54-.29-2.26,1.1-4.52,2.19-6.8,3.26a.93.93,0,0,0-.62,1c0,3.1,0,6.2,0,9.31a1.35,1.35,0,0,1-.86,1.4C67,54.27,65,55.23,63,56.17c-1,.45-1.55.11-1.63-.94,0-.21,0-.42,0-.63,0-3,0-5.94,0-8.91a.82.82,0,0,0-.55-.88c-1.39-.63-2.76-1.31-4.14-2L35.79,32.77a1,1,0,0,0-1,0c-1.2.59-2.4,1.15-3.61,1.73Zm49.67,0-.63-.32L71,34q-9.44-4.54-18.86-9.09a1,1,0,0,0-1,0c-1.91.94-3.83,1.86-5.74,2.79a1.56,1.56,0,0,0-.72.43l.56.29,8.22,3.94q7.71,3.7,15.41,7.43a1.13,1.13,0,0,0,1.14,0c.85-.43,1.71-.82,2.56-1.24Zm-39-5.1L41,33.21q10.41,5,20.81,10a1,1,0,0,0,1,0c1.26-.63,2.54-1.22,3.81-1.84.17-.09.42-.1.48-.38-.11-.06-.2-.12-.31-.17l-16.68-8-7.17-3.43a.81.81,0,0,0-.77-.12ZM63.3,49.44c0,1.33,0,2.66,0,4,0,.42.09.52.49.32,1.23-.61,2.47-1.2,3.72-1.78a.66.66,0,0,0,.42-.67c0-2.64,0-5.29,0-7.93,0-.4-.09-.52-.5-.32-1.21.6-2.43,1.19-3.66,1.76a.72.72,0,0,0-.47.76C63.31,46.86,63.3,48.15,63.3,49.44Z"/><path class="cls-1" d="M28.47,58.1a5.06,5.06,0,0,1,.53.19l7.11,3.41a1.78,1.78,0,0,1,.3.17A.85.85,0,0,1,36.79,63a.86.86,0,0,1-1.15.49,12,12,0,0,1-1.37-.62L28.22,60a1.86,1.86,0,0,1-.45-.27.85.85,0,0,1-.27-1A.93.93,0,0,1,28.47,58.1Z"/><path class="cls-1" d="M33.21,65a.88.88,0,0,1-1.13.89,6.38,6.38,0,0,1-1-.43c-1-.46-1.95-.93-2.92-1.41-.65-.32-.88-.84-.62-1.34a1,1,0,0,1,1.4-.35c1.23.57,2.44,1.16,3.66,1.75A1,1,0,0,1,33.21,65Z"/></svg>',
        /*almacen*/ "Almacén": '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><style>.cls-1{fill:#c3cf19;}</style></defs><g id="Capa_2" data-name="Capa 2"><path class="cls-1" d="M35.11,38.14V36.52c0-1.05,0-2.1,0-3.16a3,3,0,0,1,3.09-3c5,0,9.93,0,14.9,0,6.83,0,13.65,0,20.48,0A3.27,3.27,0,0,1,77,33.68c-.07,1.08,0,2.17,0,3.26a1.05,1.05,0,0,1-1.06,1.18c-.67,0-1.08-.43-1.08-1.2,0-1,0-2.07,0-3.11s-.37-1.38-1.4-1.38H63.72c-.48,0-.63.12-.63.6,0,3,0,6.11,0,9.16,0,.17,0,.34,0,.51a1.11,1.11,0,0,1-1.91.8c-.62-.59-1.24-1.18-1.83-1.8-.27-.28-.44-.26-.7,0-.57.59-1.16,1.16-1.75,1.73a1.12,1.12,0,0,1-1.73,0c-.61-.58-1.21-1.16-1.79-1.76-.24-.25-.39-.25-.63,0-.53.56-1.1,1.08-1.64,1.63a1.26,1.26,0,0,1-1.47.45C49.07,43.5,49,43,49,42.43c0-3.1,0-6.21,0-9.32,0-.68,0-.69-.72-.69H38.75c-1.17,0-1.5.33-1.5,1.5,0,2,0,3.94,0,5.91a1,1,0,0,0,.31.76q2.09,2,4.13,4.07a1.18,1.18,0,0,0,.87.35,29.52,29.52,0,0,1,4.7.17,12.06,12.06,0,0,1,4.92,1.88c1.54,1,3.06,2.15,4.53,3.3a3.35,3.35,0,0,1-.59,5.58,7.58,7.58,0,0,1-1.48.42c0,.2.16.28.26.38,1,.95,1.93,1.89,2.88,2.85a2.94,2.94,0,0,1-.39,4.77.45.45,0,0,0-.18.59,3,3,0,0,1-2.59,3.86,2,2,0,0,1-1-.08.65.65,0,0,0-1,.36,1.38,1.38,0,0,1-.19.25c.14.15.31.08.45.08q10.26,0,20.53,0c1,0,1.39-.36,1.39-1.39q0-11.18,0-22.35c0-.2,0-.41,0-.61a1.07,1.07,0,0,1,2.11,0,2.9,2.9,0,0,1,0,.51c0,7.55,0,15.11,0,22.66a3.34,3.34,0,0,1-3.41,3.34c-11.71,0-23.42,0-35.12,0a3.09,3.09,0,0,1-3.36-3.29V62.38a1.22,1.22,0,0,0-.24-.8,10,10,0,0,1-2-6.29A1.69,1.69,0,0,0,32.28,54c-2.61-2.54-5.2-5.11-7.8-7.66a1.27,1.27,0,0,1-.45-1,.91.91,0,0,1,.54-.94,1,1,0,0,1,1.19.21l1.65,1.62c.76.74,1.52,1.48,2.27,2.24.23.24.4.29.67,0,1.91-1.89,3.82-3.78,5.74-5.65.34-.32.14-.47-.08-.69L24.52,30.78a1.6,1.6,0,0,1-.5-1.11,1,1,0,0,1,1.54-.83,2.59,2.59,0,0,1,.51.43q4.28,4.22,8.56,8.42C34.75,37.82,34.89,37.94,35.11,38.14Zm9.32,9H42.3a.89.89,0,0,0-.69.27l-6.36,6.27a.78.78,0,0,0-.27.53,9,9,0,0,0,2.67,7.23c2.51,2.44,5,4.92,7.5,7.38a3.92,3.92,0,0,0,.37.34.84.84,0,0,0,1.13,0A.82.82,0,0,0,46.72,68a3.4,3.4,0,0,0-.32-.34c-.89-.87-1.78-1.74-2.66-2.63a1.07,1.07,0,0,1,1-1.83,1.42,1.42,0,0,1,.72.46l3.95,3.89c.11.11.22.22.34.32a.85.85,0,0,0,1.13,0,.88.88,0,0,0,0-1.16c-.1-.12-.22-.22-.33-.33L46.6,62.48a1.17,1.17,0,0,1-.46-1.07,1,1,0,0,1,.72-.85,1.09,1.09,0,0,1,1.18.3c1.53,1.45,3,2.93,4.5,4.43.38.39.77.78,1.18,1.14a.79.79,0,0,0,.9.18.84.84,0,0,0,.55-.74c0-.37-.22-.59-.45-.82L49.49,59.9a2.11,2.11,0,0,1-.49-.59,1,1,0,0,1,.7-1.49,1.2,1.2,0,0,1,1.2.41l4,3.92a3.31,3.31,0,0,0,.33.32.81.81,0,0,0,1.13,0,.77.77,0,0,0,.12-1.06,4.15,4.15,0,0,0-.45-.49L48.53,53.5a1.26,1.26,0,0,1-.24-1.74,1.31,1.31,0,0,1,1.81-.2c.56.34,1.1.69,1.65,1,.71.45,1.42.91,2.14,1.35a1.14,1.14,0,0,0,1.35,0,1.2,1.2,0,0,0,.46-1.32,1.23,1.23,0,0,0-.57-.75c-1.29-1-2.57-1.9-3.86-2.83A9.57,9.57,0,0,0,49.33,48,11.14,11.14,0,0,0,44.43,47.18ZM60.79,40a.62.62,0,0,0,.1-.49c0-2.17,0-4.34,0-6.52,0-.46-.13-.61-.62-.61-2.82,0-5.65,0-8.48,0-.48,0-.62.15-.61.61,0,2.18,0,4.35,0,6.52a.62.62,0,0,0,.1.49l.88-.84a1.11,1.11,0,0,1,1.76,0c.61.58,1.21,1.16,1.79,1.76.23.24.39.24.63,0,.58-.6,1.18-1.18,1.79-1.76a1.12,1.12,0,0,1,1.76,0ZM39.93,46a1.59,1.59,0,0,0-.15-.21c-.54-.54-1.1-1.07-1.64-1.62-.2-.19-.33-.08-.48.06L31.81,50c-.2.19-.19.32,0,.5.51.48,1,1,1.5,1.48.25.25.41.2.63,0l5.74-5.65ZM37.25,64.28c0,1.32,0,2.6,0,3.88,0,.88.4,1.26,1.31,1.26q1.76,0,3.51,0c.13,0,.28.07.35,0Z"/><path class="cls-1" d="M67,59.14h3.1c.77,0,1.22.37,1.26,1a1.08,1.08,0,0,1-1.17,1.15c-.93,0-1.86,0-2.79,0-1.16,0-2.31,0-3.47,0-.84,0-1.3-.4-1.33-1a1,1,0,0,1,1.14-1.1C64.86,59.13,65.94,59.14,67,59.14Z"/><path class="cls-1" d="M68.1,63.33h2.07a1.09,1.09,0,1,1,0,2.16q-2.15,0-4.29,0a1.1,1.1,0,0,1-1.16-1.11,1.12,1.12,0,0,1,1.18-1c.72,0,1.45,0,2.17,0Z"/><path class="cls-1" d="M61.7,63.34a1.08,1.08,0,1,1,0,2.15,1.14,1.14,0,0,1-1.09-1.08A1.12,1.12,0,0,1,61.7,63.34Z"/><path class="cls-1" d="M75.92,42.2a1,1,0,0,1-1.08-1.05,1.07,1.07,0,1,1,1.08,1.05Z"/></g></svg>',
        /*transito*/ "Tránsito": '<svg id="Capa_1" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><style>.cls-1{fill:#264782;}</style></defs><rect class="cls-1" x="48.13" y="50.85" width="14.68" height="2.55"/><rect class="cls-1" x="50.97" y="56.04" width="8.51" height="1.96"/><rect class="cls-1" x="47.27" y="32.64" width="8.59" height="1.57"/><path class="cls-1" d="M68.2,62.17V58.6H66.35v3.57h-22l.11-23.53,0,1.79h6.09v7.3a.49.49,0,0,0,.49.5h1a.82.82,0,0,0,.57-.24l2-2a.94.94,0,0,1,1.28-.06L58.33,48a1.12,1.12,0,0,0,.72.26h.67a.51.51,0,0,0,.51-.51V42.21H58.5l-.07,2.6a.37.37,0,0,1-.62.27l-1.62-1.44a1.11,1.11,0,0,0-1.44,0L53,45.12a.36.36,0,0,1-.59-.27V40.43h14V55.24h2.06V38.64H60.23V22.07H42.79L42.84,29a1.87,1.87,0,0,0-.9-1.44L30.43,20.7A3.23,3.23,0,0,0,26,21.86h0a3,3,0,0,0,1.08,4.06l10.68,6.23a1.64,1.64,0,0,1,.83,1.47L38.39,44a.79.79,0,0,0,.79.81h1.24a.66.66,0,0,0,.67-.67L40.9,32.8a1.56,1.56,0,0,0-.73-1.29L28.11,24.05a1.08,1.08,0,0,1-.31-1.53A.91.91,0,0,1,29,22.27l12.4,7.53a2.69,2.69,0,0,1,.7.6,1.66,1.66,0,0,1,.39,1.05l.37,35a7.41,7.41,0,0,0-2.09-.73L40.6,48.1a.7.7,0,0,0-.71-.68h-.74a.67.67,0,0,0-.65.67V65.68a7.2,7.2,0,1,0,8.21,7.89H68.29a1.91,1.91,0,0,0,1.91-1.9V69.92h3.87V62.17Zm-6.28,1.67h4.43v4.42H61.92Zm-6,0h4.43v4.42H55.91Zm-5.86,0h4.42v4.42H50.05Zm-1.31-.18v4.43h-3.8c-.2-.22-.4-.42-.62-.62l0-3.81ZM49.08,28h.06v-3.4h5v3.84H49.08Zm-4.58-3.4h2.77V30h8.59V24.63h2.47l.17,14h-14l0-9.66Zm-5,54.1a5.94,5.94,0,0,1,0-11.87,5.84,5.84,0,0,1,1.68.25l.16,0a5.93,5.93,0,0,1-1.84,11.57ZM68,70.15a.68.68,0,0,1,.19.47V71a.69.69,0,0,1-.43.65.58.58,0,0,1-.26.06l-20.84.08a7,7,0,0,0-.54-1.82H68Zm4.4-1.89H68V63.84h4.43Z"/><path class="cls-1" d="M39.55,69.47a3.33,3.33,0,1,0,3.34,3.33A3.34,3.34,0,0,0,39.55,69.47Zm-.09,5.4a2.12,2.12,0,1,1,2.12-2.12A2.12,2.12,0,0,1,39.46,74.87Z"/></svg>',
        /*completado*/ "Completada": '<svg id="Capa_1" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100"><defs><style>.cls-1{fill:#1d3f87;}</style></defs><path class="cls-1" d="M77.66,53l-4.94-7.46a.68.68,0,0,0-.57-.3H66.29a.61.61,0,0,0-.62.6l.14,7.71a.63.63,0,0,0,.64.61H77A.75.75,0,0,0,77.66,53Zm-3.6-1H67.65V47h3.91l2.76,4.5A.32.32,0,0,1,74.06,52Z"/><path class="cls-1" d="M77.85,65.56h3.3a.84.84,0,0,0,.85-.83V54.57a2.47,2.47,0,0,0-.46-1.44L74.2,42.79A1.83,1.83,0,0,0,72.72,42H64.35V35.64A1.38,1.38,0,0,0,63,34.25H27.14a1.58,1.58,0,0,0-1.57,1.59v5.59H17.72a1,1,0,0,0,0,2h7.85v1.74H22.16a1,1,0,0,0,0,2h3.41v1.75H17.72a1,1,0,0,0,0,2h7.85v1.79H22.16a1,1,0,0,0,0,2h3.41v10a.91.91,0,0,0,.9.92h4.79A5.85,5.85,0,0,0,35,70.34H17.74a1,1,0,0,0-1,1h0a1,1,0,0,0,1,1H81a1,1,0,0,0,1-1h0a1,1,0,0,0-1-1H73.74A5.82,5.82,0,0,0,77.85,65.56ZM64.35,43.85h8.31a1.44,1.44,0,0,1,1.2.64l5.94,8.75a2.15,2.15,0,0,1,.38,1.22v8.61a.69.69,0,0,1-.69.7H77.86a5.66,5.66,0,0,0-11.25,0H64.35ZM31.29,63.77H27.91V54.61h7.3a1,1,0,0,0,0-2h-7.3V50.84h2.86a1,1,0,0,0,0-2H27.91V47.12h7.3a1,1,0,0,0,0-2h-7.3V43.41h2.86a1,1,0,0,0,0-2H27.91V36.32H62.47V63.77h-20a5.65,5.65,0,0,0-11.2,0Zm1.77,1.79a4,4,0,0,1-.11-.91,3.6,3.6,0,0,1,.1-.88,3.93,3.93,0,0,1,7.69,0,4.17,4.17,0,0,1,.09.88,4.61,4.61,0,0,1-.1.91,3.92,3.92,0,0,1-7.67,0Zm5.73,4.78a5.86,5.86,0,0,0,3.74-4.78H66.62a5.85,5.85,0,0,0,4.11,4.78ZM76,65.56a3.93,3.93,0,0,1-7.61,0,4,4,0,0,1-.14-1,4.53,4.53,0,0,1,.07-.74,3.93,3.93,0,0,1,7.75,0,4.53,4.53,0,0,1,.07.74A4,4,0,0,1,76,65.56Z"/></svg>',
        /*DEFAULT*/ "Default": '<svg id="Capa_1" data-name="Capa 1" xmlns="http://www.w3.org/2000/svg" fill="#264782" viewBox="0 0 100 100"><defs><style>.cls-1{fill:#264782;}</style></defs><path class="cls-1" d="M51.18,82a26.1,26.1,0,0,0-2.59-1.3Q37,75.13,25.47,69.59c-2-1-1.76-.69-1.76-2.79q0-14.89,0-29.78a1.41,1.41,0,0,1,.91-1.44q8-3.82,16-7.68L50.82,23a1.73,1.73,0,0,1,1.65,0q11,5.31,22,10.59c1.4.67,2.79,1.35,4.2,2A1.4,1.4,0,0,1,79.59,37q0,15.38,0,30.76a1.4,1.4,0,0,1-.93,1.42q-11.41,5.46-22.81,11c-1.25.6-2.54,1.15-3.74,1.85ZM26.81,36.62l.51.27L45.05,45.4c.89.43,1.17.91.89,1.48s-.82.64-1.72.2l-18-8.67c-.63-.31-.63-.31-.63.39q0,14,0,27.93a1.13,1.13,0,0,0,.74,1.19Q38.15,73.59,50,79.32c.67.32.67.32.67-.41v-28a1.08,1.08,0,0,1,0-.18.7.7,0,0,0-.49-.81c-.53-.22-1.05-.49-1.57-.75s-.85-.79-.61-1.3.78-.66,1.45-.36c.49.23,1,.44,1.46.72a1.35,1.35,0,0,0,1.41,0c1.75-.88,3.51-1.72,5.28-2.55A.94.94,0,0,1,59,46.1c.23.49,0,.94-.57,1.27-.14.08-.28.14-.42.21-1.64.79-3.27,1.59-4.92,2.36a.76.76,0,0,0-.5.8q0,14.17,0,28.34c0,.46.11.53.52.32,1.27-.63,2.54-1.23,3.82-1.84l20.19-9.7c.36-.17.64-.32.64-.83q0-14.19,0-28.39c0-.52-.17-.47-.54-.29-2.26,1.1-4.52,2.19-6.8,3.26a.93.93,0,0,0-.62,1c0,3.1,0,6.2,0,9.31a1.35,1.35,0,0,1-.86,1.4C67,54.27,65,55.23,63,56.17c-1,.45-1.55.11-1.63-.94,0-.21,0-.42,0-.63,0-3,0-5.94,0-8.91a.82.82,0,0,0-.55-.88c-1.39-.63-2.76-1.31-4.14-2L35.79,32.77a1,1,0,0,0-1,0c-1.2.59-2.4,1.15-3.61,1.73Zm49.67,0-.63-.32L71,34q-9.44-4.54-18.86-9.09a1,1,0,0,0-1,0c-1.91.94-3.83,1.86-5.74,2.79a1.56,1.56,0,0,0-.72.43l.56.29,8.22,3.94q7.71,3.7,15.41,7.43a1.13,1.13,0,0,0,1.14,0c.85-.43,1.71-.82,2.56-1.24Zm-39-5.1L41,33.21q10.41,5,20.81,10a1,1,0,0,0,1,0c1.26-.63,2.54-1.22,3.81-1.84.17-.09.42-.1.48-.38-.11-.06-.2-.12-.31-.17l-16.68-8-7.17-3.43a.81.81,0,0,0-.77-.12ZM63.3,49.44c0,1.33,0,2.66,0,4,0,.42.09.52.49.32,1.23-.61,2.47-1.2,3.72-1.78a.66.66,0,0,0,.42-.67c0-2.64,0-5.29,0-7.93,0-.4-.09-.52-.5-.32-1.21.6-2.43,1.19-3.66,1.76a.72.72,0,0,0-.47.76C63.31,46.86,63.3,48.15,63.3,49.44Z"/><path class="cls-1" d="M28.47,58.1a5.06,5.06,0,0,1,.53.19l7.11,3.41a1.78,1.78,0,0,1,.3.17A.85.85,0,0,1,36.79,63a.86.86,0,0,1-1.15.49,12,12,0,0,1-1.37-.62L28.22,60a1.86,1.86,0,0,1-.45-.27.85.85,0,0,1-.27-1A.93.93,0,0,1,28.47,58.1Z"/><path class="cls-1" d="M33.21,65a.88.88,0,0,1-1.13.89,6.38,6.38,0,0,1-1-.43c-1-.46-1.95-.93-2.92-1.41-.65-.32-.88-.84-.62-1.34a1,1,0,0,1,1.4-.35c1.23.57,2.44,1.16,3.66,1.75A1,1,0,0,1,33.21,65Z"/></svg>',
    }

    useEffect(() => {
        const { estados } = informacionPedido;
        const estadoAcutal = informacionPedido.pedido.status;

        const updatedData = estados.map((estado, index) => {
            let clase = estado.statusId === estadoAcutal ? 'actual' : (index < estados.findIndex(e => e.statusId === estadoAcutal) ? 'completado' : 'pendiente');

            if(estadoAcutal === 7 && estado.statusId === estadoAcutal){
                clase = 'finalizado';
            }

            const claseCancelada = (clase === 'actual' && (estadoAcutal === 14 || estadoAcutal === 12)) ? 'cancelado' : null;
            return {
                clase: isNil(claseCancelada) ? clase : claseCancelada, 
                img: iconosImg[estado.status] ?? iconosImg['Default'],
                estado: estado.status,
                detalles: [new Date(estado.created_at).toLocaleString('es-GT', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })]
            };
        });

        const filteredData = updatedData.filter((data, index) => {
            if (data.estado === "Liquidado" && data.clase === 'completado') {
                return false;
            }
            if (data.estado === 'Completada' && data.clase === 'completado' && estadoAcutal !== 7) { //Estado Liquidado
                return false;
            }
            if (data.estado === 'Devolución' && data.clase === 'completado') {
                return false;
            }
            if (data.estado === 'Creado' && data.clase === 'completado') {
                return false;
            }
            if (data.estado === 'Cancelado' && data.clase === 'completado' && (estadoAcutal !== 14 || estadoAcutal !== 12)) {
                return false;
            }
            return true;
        });

        const requiredStates = ['Recibido', 'Almacén', 'Completada'];
        if (estadoAcutal !== 14 && estadoAcutal !== 12) {
            requiredStates.forEach(state => {
                if (!filteredData.some(d => d.estado === state)) {
                    filteredData.push({
                        clase: state === 'Recibido' ? 'actual' : 'pendiente',
                        img: iconosImg[state] ?? iconosImg['Default'],
                        estado: state,
                        detalles: state === 'Recibido' ? [new Date().toLocaleString('es-GT', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })] : []
                    });
                }
            });
        }

        // Ensure 'Recibido' is always the first element
        const recibidoIndex = filteredData.findIndex(d => d.estado === 'Recibido');
        if (recibidoIndex > 0) {
            const [recibido] = filteredData.splice(recibidoIndex, 1);
            filteredData.unshift(recibido);
        }
        

        setData(filteredData);

    }, [informacionPedido]);

    return (
        <CContainer className='container-tracking-events'>
            {
                data.map(d => (
                    <CRow className={`rowEvent ${d.clase}`}>
                        <CCol className={'colIconEvent'} style={{ padding: 0 }}>
                            {typeof d.img === 'string' ? (
                                <div dangerouslySetInnerHTML={{ __html: d.img }} className={d.clase}>
                                </div>
                            ) : (
                                <div className={`${d.clase}`}>{d.img()}</div>
                            )}
                        </CCol>
                        <CCol xs={8} sm={8} md={8} lg={10} className={'colDataEvent'}>
                            <div>
                                <span>
                                    <strong>{d.estado}</strong>
                                </span>
                            </div>
                            {
                                d.detalles.map((det) => (
                                    <div>
                                        <span>{det}</span>
                                    </div>
                                ))
                            }
                        </CCol>
                    </CRow>
                ))

            }

        </CContainer>
    )

}

export default Event;